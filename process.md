...
### 模块十三：报表与导出 ✅ 2026-07-18

**改动摘要**：3个统计报表（基金收支/费用构成/就诊统计）按年度月份聚合查询。

**改动文件清单**：

| 层级 | 文件 | 改动 |
|------|------|------|
| Service | `IReportService.java` + Impl | **新增**：fundReport(按医院聚合统筹+大病+救助)/feeCompositionReport(药品/检查/治疗/耗材分类)/visitStatsReport(门诊住院人次/人均费用/报销比例) |
| Controller | `ReportController.java` | **新增**：GET /report/fund / GET /report/fee-composition / GET /report/visit-stats |

**技术点落实**：
- 基金收支报表：按年月筛选已拨付结算，按医院聚合统筹+大病+救助总额
- 费用构成：按费用名称关键词自动分类（片/胶囊→药品，检查/镜/心电图→检查，手术/护理→治疗）
- 就诊统计：门诊/住院按visit.type区分；人均费用 = totalFee/settleCount；报销比例 = totalReimburse/totalFee × 100%
- @Permission：报表接口限医保局/管理员

**遗漏风险**：
- 费用分类基于名称关键词匹配，不够精确（生产环境应收口 catalog 表的 project_type 字段）
- 无 Excel/PDF 导出功能

---

### 模块十四：数据安全与审计 ✅ 2026-07-18

**改动摘要**：操作日志持久化到数据库（非仅注解日志），请求参数过滤敏感对象。

**改动文件清单**：

| 层级 | 文件 | 改动 |
|------|------|------|
| DDL | `int.sql` | 新增 `operation_log` 表 |
| PO | `OperationLog.java` | **新增**：userId/operation/method/params/result/durationMs |
| Mapper | `OperationLogMapper.java` | **新增** |
| Aspect | `OperationLogAspect.java` | 重写：Around 通知增加 persistLog()，请求参数过滤 HttpServletRequest/Response；成功/失败均入库；入库失败 catch 不抛异常 |

**技术点落实**：
- 双写：控制台SLF4J + 数据库表双写
- 参数过滤：过滤 Servlet 对象避免序列化循环引用和体积爆炸
- 参数截断：params>500截断，result>200截断
- 异常安全：入库失败仅log.warn不抛异常，不影响业务主流程
- 全接口覆盖：所有带 @OperationLog 注解的方法自动持久化
