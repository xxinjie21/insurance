...
[核心特性](#-核心特性) • [技术栈](#-技术栈) • [快速开始](#-快速开始) • [API 接口](#-api-接口) • [数据库设计](#-数据库设计) • [项目结构](#-项目结构) • [面试考点](#-面试考点)
...
**参考答案**：
> 1. **问题**：声明式事务回滚时，已写入的 Redis 幂等缓存无法撤销
...
**参考答案**：
> 1. **问题**：全局锁导致无关单据并发阻塞
> 2. **解决**：`lock:settle:{visitId}` 单据级锁粒度
> 3. **效果**：仅串行化同一单据的并发请求
> 4. **锁超时**：30 秒租约，防死锁
...
**参考答案**：
> 1. **Redis 预校验**：`idempotent:settle:visit:{visitId}` 判断已结算
> 2. **分布式锁**：Redisson 锁住 visitId
> 3. **唯一索引**：`uk_visit_id` 数据库兜底
> 4. **三层递进**：任一层拦截即可保证幂等
...
**参考答案**：
> 1. **事务性 Outbox**：业务事务内写入 `mq_outbox` 表
> 2. **Confirm 回调**：RabbitMQ 确认后更新 Outbox 状态
> 3. **幂等消费**：Redisson `idempotent:mq:{messageId}` 防重复
> 4. **定时重试**：`MqRetryJob` 每 5 分钟扫描重试
